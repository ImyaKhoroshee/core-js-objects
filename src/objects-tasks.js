...
function mergeObjects(objects) {
  ...
  const allEntries = [
    Object.entries(objects[0]),
    Object.entries(objects[1]),
  ].flat();

  allEntries.forEach((array) => {
    if (array[0] in mergedObj) {
      mergedObj[array[0]] += array[1];
    } else {
      const key = array[0];
      const value = array[1];
      mergedObj[key] = value;
...
function makeWord(lettersObject) {
  ...
  keys.forEach((key) => {
    const values = lettersObject[key];
